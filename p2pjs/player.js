					...
					customType: {
						'customHls': function(video, player) {
							...
							engine.on('stats', function(data) {
								$('.load').text(' 加速' + (data.totalHTTPDownloaded / 1024).toFixed(2) + 'MB');
								data.totalP2PDownloaded ? $('.peer').text('P2P加速中') : $('.peer').text('全民加速引擎');
							});
						}
					}
				}
...
console.log("%cP2P视频解析加速播放","color: red; font-size: 20px");
